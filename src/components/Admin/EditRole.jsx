import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { useSnackbar } from 'notistack';
import { Card, CardContent, TextField, Button, Box, Grid } from '@mui/material';
import ArrowBackIcon from '@mui/icons-material/ArrowBack';
import { useDispatch, useSelector } from 'react-redux';
import { getRoleDetails, updateRole, clearErrors } from '../../actions/rolesActions';
import { UPDATE_ROLE_RESET } from '../../constants/rolesConstants';

import Swal from 'sweetalert2'

const EditRole = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { enqueueSnackbar } = useSnackbar();
    const { id } = useParams();

    const [roleForm, setRoleForm] = useState({ name: '' });
    const [validation, setValidation] = useState({});

    // Redux selectors
    const { loading: updateLoading, error: updateError, isUpdated } = useSelector(
        (state) => state.updateRole || {}
    );

    const { role, loading: detailsLoading, error: detailsError } = useSelector(
        (state) => state.roleDetails || {}
    );

    // -----------------------------------------
    // Prefill form & handle API responses
    // -----------------------------------------
    useEffect(() => {
        dispatch(getRoleDetails(id));
    }, [dispatch, id]);

    useEffect(() => {
        if (role && role._id === id) {
            setRoleForm({ name: role.name });
        }
    }, [role, id]);

    useEffect(() => {
        if (detailsError) {
            Swal.fire({
                title: "Failed!",
                text: "Role details not found!",
                icon: "error",
                timer: 2000,
                showConfirmButton: false
            });
            dispatch(clearErrors());
        }

        if (updateError) {
            Swal.fire({
                title: "Failed!",
                text: "Role Edit Failed!",
                icon: "error",
                timer: 2000,
                showConfirmButton: false
            });
            dispatch(clearErrors());
        }

        if (isUpdated) {
            Swal.fire({
                title: "Success!",
                text: "Role updated successfully!",
                icon: "success",
                timer: 2000,
                showConfirmButton: false
            });
            navigate('/admin/roles');
            dispatch({ type: UPDATE_ROLE_RESET });
        }
    }, [dispatch, detailsError, updateError, isUpdated, navigate]);

    // -----------------------------------------
    // Validate form
    // -----------------------------------------
    const validateForm = () => {
        let errors = {};
        if (!roleForm.name.trim()) errors.name = "Role name is required";
        const namePattern = /^[A-Z][A-Za-z\s]*$/;
        if (!namePattern.test(roleForm.name)) {
            errors.name = "Name should start with capital letter and doesn't contains Numbers";
        }
        setValidation(errors);
        return Object.keys(errors).length === 0;
    };

    // -----------------------------------------
    // Handle Submit
    // -----------------------------------------
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!validateForm()) return;

        dispatch(updateRole(id, { name: roleForm.name }));
    };

    const handleCancel = () => navigate('/admin/roles');

    if (detailsLoading) return <p>Loading role details...</p>;

    return (
        <div className="min-h-screen w-full flex justify-center items-center p-6 bg-gray-50 ">
            <Card
                sx={{
                    width: "500px",
                    borderRadius: "14px",
                    boxShadow: "0 4px 24px rgba(0,0,0,0.08)"
                }}
            >

                <h2 className="text-2xl font-bold text-gray-800 text-center mt-4 mb-4">
                    Edit Role
                </h2>

                <CardContent sx={{ padding: "28px 40px" }}>
                    <Box component="form" onSubmit={handleSubmit}>
                        <Grid container spacing={3}>
                            {/* Role Name */}
                            <Grid item xs={12}>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Role Name *
                                </label>
                                <TextField
                                    fullWidth
                                    variant="outlined"
                                    placeholder="Enter role name"
                                    value={roleForm.name}
                                    onChange={(e) =>
                                        setRoleForm({ name: e.target.value })
                                    }
                                    error={Boolean(validation.name)}
                                    helperText={validation.name}
                                />
                            </Grid>
                        </Grid>

                        {/* Buttons */}
                        <Box className="flex justify-end gap-4 mt-6">
                            <Button
                                variant="outlined"
                                disabled={updateLoading}
                                onClick={handleCancel}
                                sx={{ minWidth: "150px" }}
                            >
                                CANCEL
                            </Button>

                            <Button
                                type="submit"
                                variant="contained"
                                disabled={updateLoading}
                                className="bg-blue-600 hover:bg-blue-700"
                                sx={{ minWidth: "150px" }}
                            >
                                {updateLoading ? "Updating..." : "UPDATE ROLE"}
                            </Button>
                        </Box>
                    </Box>
                </CardContent>
            </Card>
        </div>
    );
};

export default EditRole;
